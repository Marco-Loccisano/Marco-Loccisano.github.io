...
library(knitr)
library(ggplot2)
df <- read.csv('C:/Users/student/Documents/MATH421/Marco-Loccisano.github.io/titanic.csv')
...
df %>% group_by(Sex) %>% 
  ...
  ggplot()+ 
  geom_col(mapping=aes(x=Sex, y=average_fare))
```
...
df %>% 
  ...
  ggplot()+
  geom_col(aes(x=Pclass, y=mean_age, fill=Survived), position = 'dodge')
```
...
```{r}
df %>% 
  filter(location=='Argentina') %>%filter(date>='2022-01-01' & date<= '2022-12-31') %>% 
  ggplot()+
  geom_point(mapping = aes(x = date, 
                                    y = total_vaccinations, 
                                    color = vaccine))
...
table(df$location)
df %>% 
  filter(location %in% c('South Korea', 'Nepal', 'Japan', 'Hong Kong', 'Cyprus')) %>% 
  group_by(date, location) %>% 
  summarise(total_vac2=sum(total_vaccinations, na.rm=TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, 
...
table(df$location)
df %>% 
  filter(location %in% c('Argentina', 'Chile', 'Ecuador', 'Peru', 'Uruguay')) %>% filter(date>='2021-01-01' &date<='2021-12-31') %>% 
  group_by(date, location) %>% 
  summarise(total_vac2=sum(total_vaccinations, na.rm=TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, 
