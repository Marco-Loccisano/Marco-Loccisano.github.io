...
df %>% 
  ...
  group_by(weekend) %>% 
  summarise(Hospitalized_Increase = mean(hospitalizedIncrease))
```
...
df %>% 
  filter(weekend == 1) %>% 
  group_by(month) %>% 
  summarize(Death = max(deathIncrease)) %>% 
  arrange(-Death)
```
...
titanic %>% select(Sex, Name, Age, SibSp)
titanic %>% filter(Sex=='male')
titanic %>% mutate(Age_plus_Fare = Age + Fare)
titanic %>% summarise(mean_Age = mean(Age, na.rm=TRUE))
titanic %>% arrange(Pclass)
titanic %>% count(Sex)
titanic %>% count(Pclass) %>% arrange(-n)
titanic %>% filter(Pclass == '1') %>%  count(Sex) %>%  arrange(-n)
titanic %>% group_by(Sex) %>%  summarise(median_Pclass=median(Pclass, na.rm=TRUE))
titanic %>% filter(Sex=='male') %>%  group_by(Pclass) %>%  summarise(mean_fare=mean(Fare, na.rm=TRUE))
titanic %>% filter(Sex=='male') %>%  group_by(Survived) %>%  summarise(Pclass=median(Pclass, ne.rm=TRUE)) %>% arrange(Pclass)
```
